...
class ArgentinianReportCustomHandler(models.AbstractModel):
    __inherit = "l10n_ar.tax.report.handler.test"
    ...
    def _check_invoices(self, invoices):
        ...
        for inv in invoices:
            ...
            for vat_tax in vat_taxes:
                calculated_amount = (
                    vat_tax["BaseImp"] * l10n_ar_vat_afip_code[vat_tax["Id"]] / 100
                )
                diff = float_round(
                    abs(vat_tax["Importe"] - calculated_amount), precision_digits=2
                )
                if diff > 0.5:
                    res.append(
                        InvoiceDiff(
    ...
    def _vat_book_get_REGINFO_CV_ALICUOTAS(self, options, tax_type, invoices):
        ...
        if not error or (
            self.env.user.has_group("saas_client.group_saas_support")
            and request.session.debug
        ):
            return super()._vat_book_get_REGINFO_CV_ALICUOTAS(
                options, tax_type, invoices
            )
